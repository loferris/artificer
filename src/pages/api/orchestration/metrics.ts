...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    // Complexity distribution (from analysis)
    ...
    decisions.forEach(d => {
      const complexity = (d.analysis as any)?.complexity || 0;
      if (complexity >= 1 && complexity <= 3) {
        complexityDistribution.low++;
      } else if (complexity >= 4 && complexity <= 6) {
        complexityDistribution.medium++;
      } else if (complexity >= 7 && complexity <= 10) {
        complexityDistribution.high++;
      }
    });

    // Validation statistics
    const validatedCount = decisions.filter(d => d.validationResult !== null).length;
    ...
    const metrics = {
      ...
      recentDecisions: decisions.slice(0, 10).map(d => ({
        ...
        complexity: (d.analysis as any)?.complexity || 0,
        category: (d.analysis as any)?.category || 'unknown',
