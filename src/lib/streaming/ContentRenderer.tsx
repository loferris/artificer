...
import { ContentType, ContentSegment, StreamingStrategy, DEFAULT_CONTENT_CONFIG } from './types';
 ...
 */
export const ContentRenderer: React.FC<ContentRendererProps> = ({ segment, isStreaming }) => {
 ...
 */
const WordByWordRenderer: React.FC<ContentRendererProps> = ({ segment, isStreaming }) => {
      ...
      <div className='max-w-none'>
        <ReactMarkdown
          ...
          components={{
            ...
            h4: ({ children }) => (
              <h4 className='text-base font-bold mt-3 mb-2 text-white'>{children}</h4>
            ),
            h5: ({ children }) => (
              <h5 className='text-sm font-bold mt-2 mb-1 text-white'>{children}</h5>
            ),
            h6: ({ children }) => (
              <h6 className='text-xs font-bold mt-2 mb-1 text-white'>{children}</h6>
            ),
 ...
 */
const BufferedRenderer: React.FC<ContentRendererProps> = ({ segment, isStreaming }) => {
...
const CodeBlockRenderer: React.FC<{ segment: ContentSegment }> = ({ segment }) => {
  ...
  return (
    <div className='bg-gray-100 dark:bg-gray-800 rounded p-3 my-2'>
      {language && <div className='text-xs text-gray-500 mb-2 font-mono'>{language}</div>}
      <pre className='font-mono text-sm whitespace-pre-wrap overflow-x-auto'>
        <code>{code}</code>
      </pre>
    </div>
...
const HeaderRenderer: React.FC<{ segment: ContentSegment }> = ({ segment }) => {
  ...
  return <Tag className={classes[level as keyof typeof classes] || classes[6]}>{text}</Tag>;
};
